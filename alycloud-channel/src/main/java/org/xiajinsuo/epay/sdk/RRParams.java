/**
 * Autogenerated by Avro
 *
 * DO NOT EDIT DIRECTLY
 */

/**
 * @Copyright 2017 www.bestpay.io Inc. All rights reserved.
 */

package org.xiajinsuo.epay.sdk;

import org.apache.avro.specific.SpecificData;
import org.codehaus.jackson.map.annotate.JsonSerialize.Inclusion;

import com.fasterxml.jackson.annotation.JsonInclude.Include;

@SuppressWarnings("all")
/** 公共请求和响应 */
@javax.xml.bind.annotation.XmlRootElement
@javax.xml.bind.annotation.XmlAccessorType(javax.xml.bind.annotation.XmlAccessType.FIELD)
@javax.xml.bind.annotation.XmlType(name="RRParams", namespace="org.xiajinsuo.epay.sdk")
@org.apache.avro.specific.AvroGenerated
public class RRParams extends io.bestpay.framework.base.SpecificRecordBase implements io.bestpay.framework.base.SpecificRecord {
  private static final long serialVersionUID = 1429462467152685891L;
  public static final org.apache.avro.Schema SCHEMA$ = new org.apache.avro.Schema.Parser().parse("{\"type\":\"record\",\"name\":\"RRParams\",\"namespace\":\"org.xiajinsuo.epay.sdk\",\"doc\":\"公共请求和响应\",\"fields\":[{\"name\":\"app_id\",\"type\":{\"type\":\"string\",\"avro.java.string\":\"String\"},\"doc\":\"应用id\",\"default\":\"\"},{\"name\":\"server_trans_id\",\"type\":{\"type\":\"string\",\"avro.java.string\":\"String\"},\"doc\":\"服务器流水号\",\"default\":\"\"},{\"name\":\"client_trans_id\",\"type\":{\"type\":\"string\",\"avro.java.string\":\"String\"},\"doc\":\"客户端流水号\",\"default\":\"\"},{\"name\":\"trans_timestamp\",\"type\":\"long\",\"doc\":\"交易时间错\",\"default\":\"\"},{\"name\":\"data\",\"type\":{\"type\":\"string\",\"avro.java.string\":\"String\"},\"doc\":\"请求响应json数据\",\"default\":\"\"},{\"name\":\"resp_code\",\"type\":{\"type\":\"string\",\"avro.java.string\":\"String\"},\"doc\":\"响应状态码\",\"default\":\"\"},{\"name\":\"resp_msg\",\"type\":{\"type\":\"string\",\"avro.java.string\":\"String\"},\"doc\":\"响应消息\",\"default\":\"\"},{\"name\":\"sign\",\"type\":{\"type\":\"string\",\"avro.java.string\":\"String\"},\"doc\":\"签名\",\"default\":\"\"},{\"name\":\"trans_type\",\"type\":{\"type\":\"string\",\"avro.java.string\":\"String\"},\"doc\":\"交易类型\",\"default\":\"\"}],\"aliases\":[\"RRParams\"]}");
  public static org.apache.avro.Schema getClassSchema() { return SCHEMA$; }
  /** 应用id */
  @javax.xml.bind.annotation.XmlElement(name="app_id")
  private java.lang.String app_id;
  /** 服务器流水号 */
  @javax.xml.bind.annotation.XmlElement(name="server_trans_id")
  private java.lang.String server_trans_id;
  /** 客户端流水号 */
  @javax.xml.bind.annotation.XmlElement(name="client_trans_id")
  private java.lang.String client_trans_id;
  /** 交易时间错 */
  @javax.xml.bind.annotation.XmlElement(name="trans_timestamp")
  private java.lang.Long trans_timestamp;
  /** 请求响应json数据 */
  @javax.xml.bind.annotation.XmlElement(name="data")
  private java.lang.String data;
  /** 响应状态码 */
  @javax.xml.bind.annotation.XmlElement(name="resp_code")
  private java.lang.String resp_code;
  /** 响应消息 */
  @javax.xml.bind.annotation.XmlElement(name="resp_msg")
  private java.lang.String resp_msg;
  /** 签名 */
  @javax.xml.bind.annotation.XmlElement(name="sign")
  private java.lang.String sign;
  /** 交易类型 */
  @javax.xml.bind.annotation.XmlElement(name="trans_type")
  private java.lang.String trans_type;

  /**
   * Default constructor.  Note that this does not initialize fields
   * to their default values from the schema.  If that is desired then
   * one should use <code>newBuilder()</code>.
   */
  public RRParams() {}

  /**
   * All-args constructor.
   * @param app_id 应用id
   * @param server_trans_id 服务器流水号
   * @param client_trans_id 客户端流水号
   * @param trans_timestamp 交易时间错
   * @param data 请求响应json数据
   * @param resp_code 响应状态码
   * @param resp_msg 响应消息
   * @param sign 签名
   * @param trans_type 交易类型
   */
  public RRParams(java.lang.String app_id, java.lang.String server_trans_id, java.lang.String client_trans_id, java.lang.Long trans_timestamp, java.lang.String data, java.lang.String resp_code, java.lang.String resp_msg, java.lang.String sign, java.lang.String trans_type) {
    this.app_id = app_id;
    this.server_trans_id = server_trans_id;
    this.client_trans_id = client_trans_id;
    this.trans_timestamp = trans_timestamp;
    this.data = data;
    this.resp_code = resp_code;
    this.resp_msg = resp_msg;
    this.sign = sign;
    this.trans_type = trans_type;
  }

  @com.fasterxml.jackson.annotation.JsonIgnore
  @org.codehaus.jackson.annotate.JsonIgnore
  @javax.xml.bind.annotation.XmlTransient
  public org.apache.avro.Schema getSchema() { return SCHEMA$; }
  // Used by DatumWriter.  Applications should not call.
  public java.lang.Object get(int field$) {
    switch (field$) {
    case 0: return app_id;
    case 1: return server_trans_id;
    case 2: return client_trans_id;
    case 3: return trans_timestamp;
    case 4: return data;
    case 5: return resp_code;
    case 6: return resp_msg;
    case 7: return sign;
    case 8: return trans_type;
    default: throw new org.apache.avro.AvroRuntimeException("Bad index");
    }
  }

  // Used by DatumReader.  Applications should not call.
  @SuppressWarnings(value="unchecked")
  public void put(int field$, java.lang.Object value$) {
    switch (field$) {
    case 0: app_id = (java.lang.String)value$; break;
    case 1: server_trans_id = (java.lang.String)value$; break;
    case 2: client_trans_id = (java.lang.String)value$; break;
    case 3: trans_timestamp = (java.lang.Long)value$; break;
    case 4: data = (java.lang.String)value$; break;
    case 5: resp_code = (java.lang.String)value$; break;
    case 6: resp_msg = (java.lang.String)value$; break;
    case 7: sign = (java.lang.String)value$; break;
    case 8: trans_type = (java.lang.String)value$; break;
    default: throw new org.apache.avro.AvroRuntimeException("Bad index");
    }
  }

  /**
   * Gets the value of the 'app_id' field.
   * @return 应用id
   */
  @com.fasterxml.jackson.annotation.JsonGetter("app_id")
  @org.codehaus.jackson.annotate.JsonProperty("app_id")   
  @com.fasterxml.jackson.annotation.JsonInclude(Include.NON_NULL)
  @org.codehaus.jackson.map.annotate.JsonSerialize(include=Inclusion.NON_NULL)  
  @javax.xml.bind.annotation.XmlTransient
  public java.lang.String getAppId() {
    return app_id;
  }

  /**
   * Sets the value of the 'app_id' field.
   * 应用id
   * @param value the value to set.
   */
  protected void setAppId(java.lang.String value) {
    this.app_id = value;
  }

  /**
   * Gets the value of the 'server_trans_id' field.
   * @return 服务器流水号
   */
  @com.fasterxml.jackson.annotation.JsonGetter("server_trans_id")
  @org.codehaus.jackson.annotate.JsonProperty("server_trans_id")   
  @com.fasterxml.jackson.annotation.JsonInclude(Include.NON_NULL)
  @org.codehaus.jackson.map.annotate.JsonSerialize(include=Inclusion.NON_NULL)  
  @javax.xml.bind.annotation.XmlTransient
  public java.lang.String getServerTransId() {
    return server_trans_id;
  }

  /**
   * Sets the value of the 'server_trans_id' field.
   * 服务器流水号
   * @param value the value to set.
   */
  protected void setServerTransId(java.lang.String value) {
    this.server_trans_id = value;
  }

  /**
   * Gets the value of the 'client_trans_id' field.
   * @return 客户端流水号
   */
  @com.fasterxml.jackson.annotation.JsonGetter("client_trans_id")
  @org.codehaus.jackson.annotate.JsonProperty("client_trans_id")   
  @com.fasterxml.jackson.annotation.JsonInclude(Include.NON_NULL)
  @org.codehaus.jackson.map.annotate.JsonSerialize(include=Inclusion.NON_NULL)  
  @javax.xml.bind.annotation.XmlTransient
  public java.lang.String getClientTransId() {
    return client_trans_id;
  }

  /**
   * Sets the value of the 'client_trans_id' field.
   * 客户端流水号
   * @param value the value to set.
   */
  protected void setClientTransId(java.lang.String value) {
    this.client_trans_id = value;
  }

  /**
   * Gets the value of the 'trans_timestamp' field.
   * @return 交易时间错
   */
  @com.fasterxml.jackson.annotation.JsonGetter("trans_timestamp")
  @org.codehaus.jackson.annotate.JsonProperty("trans_timestamp")   
  @com.fasterxml.jackson.annotation.JsonInclude(Include.NON_NULL)
  @org.codehaus.jackson.map.annotate.JsonSerialize(include=Inclusion.NON_NULL)  
  @javax.xml.bind.annotation.XmlTransient
  public java.lang.Long getTransTimestamp() {
    return trans_timestamp;
  }

  /**
   * Sets the value of the 'trans_timestamp' field.
   * 交易时间错
   * @param value the value to set.
   */
  protected void setTransTimestamp(java.lang.Long value) {
    this.trans_timestamp = value;
  }

  /**
   * Gets the value of the 'data' field.
   * @return 请求响应json数据
   */
  @com.fasterxml.jackson.annotation.JsonGetter("data")
  @org.codehaus.jackson.annotate.JsonProperty("data")   
  @com.fasterxml.jackson.annotation.JsonInclude(Include.NON_NULL)
  @org.codehaus.jackson.map.annotate.JsonSerialize(include=Inclusion.NON_NULL)  
  @javax.xml.bind.annotation.XmlTransient
  public java.lang.String getData() {
    return data;
  }

  /**
   * Sets the value of the 'data' field.
   * 请求响应json数据
   * @param value the value to set.
   */
  protected void setData(java.lang.String value) {
    this.data = value;
  }

  /**
   * Gets the value of the 'resp_code' field.
   * @return 响应状态码
   */
  @com.fasterxml.jackson.annotation.JsonGetter("resp_code")
  @org.codehaus.jackson.annotate.JsonProperty("resp_code")   
  @com.fasterxml.jackson.annotation.JsonInclude(Include.NON_NULL)
  @org.codehaus.jackson.map.annotate.JsonSerialize(include=Inclusion.NON_NULL)  
  @javax.xml.bind.annotation.XmlTransient
  public java.lang.String getRespCode() {
    return resp_code;
  }

  /**
   * Sets the value of the 'resp_code' field.
   * 响应状态码
   * @param value the value to set.
   */
  protected void setRespCode(java.lang.String value) {
    this.resp_code = value;
  }

  /**
   * Gets the value of the 'resp_msg' field.
   * @return 响应消息
   */
  @com.fasterxml.jackson.annotation.JsonGetter("resp_msg")
  @org.codehaus.jackson.annotate.JsonProperty("resp_msg")   
  @com.fasterxml.jackson.annotation.JsonInclude(Include.NON_NULL)
  @org.codehaus.jackson.map.annotate.JsonSerialize(include=Inclusion.NON_NULL)  
  @javax.xml.bind.annotation.XmlTransient
  public java.lang.String getRespMsg() {
    return resp_msg;
  }

  /**
   * Sets the value of the 'resp_msg' field.
   * 响应消息
   * @param value the value to set.
   */
  protected void setRespMsg(java.lang.String value) {
    this.resp_msg = value;
  }

  /**
   * Gets the value of the 'sign' field.
   * @return 签名
   */
  @com.fasterxml.jackson.annotation.JsonGetter("sign")
  @org.codehaus.jackson.annotate.JsonProperty("sign")   
  @com.fasterxml.jackson.annotation.JsonInclude(Include.NON_NULL)
  @org.codehaus.jackson.map.annotate.JsonSerialize(include=Inclusion.NON_NULL)  
  @javax.xml.bind.annotation.XmlTransient
  public java.lang.String getSign() {
    return sign;
  }

  /**
   * Sets the value of the 'sign' field.
   * 签名
   * @param value the value to set.
   */
  protected void setSign(java.lang.String value) {
    this.sign = value;
  }

  /**
   * Gets the value of the 'trans_type' field.
   * @return 交易类型
   */
  @com.fasterxml.jackson.annotation.JsonGetter("trans_type")
  @org.codehaus.jackson.annotate.JsonProperty("trans_type")   
  @com.fasterxml.jackson.annotation.JsonInclude(Include.NON_NULL)
  @org.codehaus.jackson.map.annotate.JsonSerialize(include=Inclusion.NON_NULL)  
  @javax.xml.bind.annotation.XmlTransient
  public java.lang.String getTransType() {
    return trans_type;
  }

  /**
   * Sets the value of the 'trans_type' field.
   * 交易类型
   * @param value the value to set.
   */
  protected void setTransType(java.lang.String value) {
    this.trans_type = value;
  }

  /**
   * Creates a new RRParams RecordBuilder.
   * @return A new RRParams RecordBuilder
   */
  public static org.xiajinsuo.epay.sdk.RRParams.Builder newBuilder() {
    return new org.xiajinsuo.epay.sdk.RRParams.Builder();
  }

  /**
   * Creates a new RRParams RecordBuilder by copying an existing Builder.
   * @param other The existing builder to copy.
   * @return A new RRParams RecordBuilder
   */
  public static org.xiajinsuo.epay.sdk.RRParams.Builder newBuilder(org.xiajinsuo.epay.sdk.RRParams.Builder other) {
    return new org.xiajinsuo.epay.sdk.RRParams.Builder(other);
  }

  /**
   * Creates a new RRParams RecordBuilder by copying an existing RRParams instance.
   * @param other The existing instance to copy.
   * @return A new RRParams RecordBuilder
   */
  public static org.xiajinsuo.epay.sdk.RRParams.Builder newBuilder(org.xiajinsuo.epay.sdk.RRParams other) {
    return new org.xiajinsuo.epay.sdk.RRParams.Builder(other);
  }

  /**
   * RecordBuilder for RRParams instances.
   */
  public static class Builder extends org.apache.avro.specific.SpecificRecordBuilderBase<RRParams>
    implements org.apache.avro.data.RecordBuilder<RRParams> {

    /** 应用id */
    private java.lang.String app_id;
    /** 服务器流水号 */
    private java.lang.String server_trans_id;
    /** 客户端流水号 */
    private java.lang.String client_trans_id;
    /** 交易时间错 */
    private java.lang.Long trans_timestamp;
    /** 请求响应json数据 */
    private java.lang.String data;
    /** 响应状态码 */
    private java.lang.String resp_code;
    /** 响应消息 */
    private java.lang.String resp_msg;
    /** 签名 */
    private java.lang.String sign;
    /** 交易类型 */
    private java.lang.String trans_type;

    /** Creates a new Builder */
    private Builder() {
      super(SCHEMA$);
    }

    /**
     * Creates a Builder by copying an existing Builder.
     * @param other The existing Builder to copy.
     */
    private Builder(org.xiajinsuo.epay.sdk.RRParams.Builder other) {
      super(other);
      if (isValidValue(fields()[0], other.app_id)) {
        this.app_id = data().deepCopy(fields()[0].schema(), other.app_id);
        fieldSetFlags()[0] = true;
      }
      if (isValidValue(fields()[1], other.server_trans_id)) {
        this.server_trans_id = data().deepCopy(fields()[1].schema(), other.server_trans_id);
        fieldSetFlags()[1] = true;
      }
      if (isValidValue(fields()[2], other.client_trans_id)) {
        this.client_trans_id = data().deepCopy(fields()[2].schema(), other.client_trans_id);
        fieldSetFlags()[2] = true;
      }
      if (isValidValue(fields()[3], other.trans_timestamp)) {
        this.trans_timestamp = data().deepCopy(fields()[3].schema(), other.trans_timestamp);
        fieldSetFlags()[3] = true;
      }
      if (isValidValue(fields()[4], other.data)) {
        this.data = data().deepCopy(fields()[4].schema(), other.data);
        fieldSetFlags()[4] = true;
      }
      if (isValidValue(fields()[5], other.resp_code)) {
        this.resp_code = data().deepCopy(fields()[5].schema(), other.resp_code);
        fieldSetFlags()[5] = true;
      }
      if (isValidValue(fields()[6], other.resp_msg)) {
        this.resp_msg = data().deepCopy(fields()[6].schema(), other.resp_msg);
        fieldSetFlags()[6] = true;
      }
      if (isValidValue(fields()[7], other.sign)) {
        this.sign = data().deepCopy(fields()[7].schema(), other.sign);
        fieldSetFlags()[7] = true;
      }
      if (isValidValue(fields()[8], other.trans_type)) {
        this.trans_type = data().deepCopy(fields()[8].schema(), other.trans_type);
        fieldSetFlags()[8] = true;
      }
    }

    /**
     * Creates a Builder by copying an existing RRParams instance
     * @param other The existing instance to copy.
     */
    private Builder(org.xiajinsuo.epay.sdk.RRParams other) {
            super(SCHEMA$);
      if (isValidValue(fields()[0], other.app_id)) {
        this.app_id = data().deepCopy(fields()[0].schema(), other.app_id);
        fieldSetFlags()[0] = true;
      }
      if (isValidValue(fields()[1], other.server_trans_id)) {
        this.server_trans_id = data().deepCopy(fields()[1].schema(), other.server_trans_id);
        fieldSetFlags()[1] = true;
      }
      if (isValidValue(fields()[2], other.client_trans_id)) {
        this.client_trans_id = data().deepCopy(fields()[2].schema(), other.client_trans_id);
        fieldSetFlags()[2] = true;
      }
      if (isValidValue(fields()[3], other.trans_timestamp)) {
        this.trans_timestamp = data().deepCopy(fields()[3].schema(), other.trans_timestamp);
        fieldSetFlags()[3] = true;
      }
      if (isValidValue(fields()[4], other.data)) {
        this.data = data().deepCopy(fields()[4].schema(), other.data);
        fieldSetFlags()[4] = true;
      }
      if (isValidValue(fields()[5], other.resp_code)) {
        this.resp_code = data().deepCopy(fields()[5].schema(), other.resp_code);
        fieldSetFlags()[5] = true;
      }
      if (isValidValue(fields()[6], other.resp_msg)) {
        this.resp_msg = data().deepCopy(fields()[6].schema(), other.resp_msg);
        fieldSetFlags()[6] = true;
      }
      if (isValidValue(fields()[7], other.sign)) {
        this.sign = data().deepCopy(fields()[7].schema(), other.sign);
        fieldSetFlags()[7] = true;
      }
      if (isValidValue(fields()[8], other.trans_type)) {
        this.trans_type = data().deepCopy(fields()[8].schema(), other.trans_type);
        fieldSetFlags()[8] = true;
      }
    }

    /**
      * Gets the value of the 'app_id' field.
      * 应用id
      * @return The value.
      */
    public java.lang.String getAppId() {
      return app_id;
    }

    /**
      * Sets the value of the 'app_id' field.
      * 应用id
      * @param value The value of 'app_id'.
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder setAppId(java.lang.String value) {
      validate(fields()[0], value);
      this.app_id = value;
      fieldSetFlags()[0] = true;
      return this;
    }

    /**
      * Checks whether the 'app_id' field has been set.
      * 应用id
      * @return True if the 'app_id' field has been set, false otherwise.
      */
    public boolean hasAppId() {
      return fieldSetFlags()[0];
    }


    /**
      * Clears the value of the 'app_id' field.
      * 应用id
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder clearAppId() {
      app_id = null;
      fieldSetFlags()[0] = false;
      return this;
    }

    /**
      * Gets the value of the 'server_trans_id' field.
      * 服务器流水号
      * @return The value.
      */
    public java.lang.String getServerTransId() {
      return server_trans_id;
    }

    /**
      * Sets the value of the 'server_trans_id' field.
      * 服务器流水号
      * @param value The value of 'server_trans_id'.
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder setServerTransId(java.lang.String value) {
      validate(fields()[1], value);
      this.server_trans_id = value;
      fieldSetFlags()[1] = true;
      return this;
    }

    /**
      * Checks whether the 'server_trans_id' field has been set.
      * 服务器流水号
      * @return True if the 'server_trans_id' field has been set, false otherwise.
      */
    public boolean hasServerTransId() {
      return fieldSetFlags()[1];
    }


    /**
      * Clears the value of the 'server_trans_id' field.
      * 服务器流水号
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder clearServerTransId() {
      server_trans_id = null;
      fieldSetFlags()[1] = false;
      return this;
    }

    /**
      * Gets the value of the 'client_trans_id' field.
      * 客户端流水号
      * @return The value.
      */
    public java.lang.String getClientTransId() {
      return client_trans_id;
    }

    /**
      * Sets the value of the 'client_trans_id' field.
      * 客户端流水号
      * @param value The value of 'client_trans_id'.
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder setClientTransId(java.lang.String value) {
      validate(fields()[2], value);
      this.client_trans_id = value;
      fieldSetFlags()[2] = true;
      return this;
    }

    /**
      * Checks whether the 'client_trans_id' field has been set.
      * 客户端流水号
      * @return True if the 'client_trans_id' field has been set, false otherwise.
      */
    public boolean hasClientTransId() {
      return fieldSetFlags()[2];
    }


    /**
      * Clears the value of the 'client_trans_id' field.
      * 客户端流水号
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder clearClientTransId() {
      client_trans_id = null;
      fieldSetFlags()[2] = false;
      return this;
    }

    /**
      * Gets the value of the 'trans_timestamp' field.
      * 交易时间错
      * @return The value.
      */
    public java.lang.Long getTransTimestamp() {
      return trans_timestamp;
    }

    /**
      * Sets the value of the 'trans_timestamp' field.
      * 交易时间错
      * @param value The value of 'trans_timestamp'.
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder setTransTimestamp(java.lang.Long value) {
      validate(fields()[3], value);
      this.trans_timestamp = value;
      fieldSetFlags()[3] = true;
      return this;
    }

    /**
      * Checks whether the 'trans_timestamp' field has been set.
      * 交易时间错
      * @return True if the 'trans_timestamp' field has been set, false otherwise.
      */
    public boolean hasTransTimestamp() {
      return fieldSetFlags()[3];
    }


    /**
      * Clears the value of the 'trans_timestamp' field.
      * 交易时间错
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder clearTransTimestamp() {
      fieldSetFlags()[3] = false;
      return this;
    }

    /**
      * Gets the value of the 'data' field.
      * 请求响应json数据
      * @return The value.
      */
    public java.lang.String getData() {
      return data;
    }

    /**
      * Sets the value of the 'data' field.
      * 请求响应json数据
      * @param value The value of 'data'.
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder setData(java.lang.String value) {
      validate(fields()[4], value);
      this.data = value;
      fieldSetFlags()[4] = true;
      return this;
    }

    /**
      * Checks whether the 'data' field has been set.
      * 请求响应json数据
      * @return True if the 'data' field has been set, false otherwise.
      */
    public boolean hasData() {
      return fieldSetFlags()[4];
    }


    /**
      * Clears the value of the 'data' field.
      * 请求响应json数据
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder clearData() {
      data = null;
      fieldSetFlags()[4] = false;
      return this;
    }

    /**
      * Gets the value of the 'resp_code' field.
      * 响应状态码
      * @return The value.
      */
    public java.lang.String getRespCode() {
      return resp_code;
    }

    /**
      * Sets the value of the 'resp_code' field.
      * 响应状态码
      * @param value The value of 'resp_code'.
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder setRespCode(java.lang.String value) {
      validate(fields()[5], value);
      this.resp_code = value;
      fieldSetFlags()[5] = true;
      return this;
    }

    /**
      * Checks whether the 'resp_code' field has been set.
      * 响应状态码
      * @return True if the 'resp_code' field has been set, false otherwise.
      */
    public boolean hasRespCode() {
      return fieldSetFlags()[5];
    }


    /**
      * Clears the value of the 'resp_code' field.
      * 响应状态码
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder clearRespCode() {
      resp_code = null;
      fieldSetFlags()[5] = false;
      return this;
    }

    /**
      * Gets the value of the 'resp_msg' field.
      * 响应消息
      * @return The value.
      */
    public java.lang.String getRespMsg() {
      return resp_msg;
    }

    /**
      * Sets the value of the 'resp_msg' field.
      * 响应消息
      * @param value The value of 'resp_msg'.
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder setRespMsg(java.lang.String value) {
      validate(fields()[6], value);
      this.resp_msg = value;
      fieldSetFlags()[6] = true;
      return this;
    }

    /**
      * Checks whether the 'resp_msg' field has been set.
      * 响应消息
      * @return True if the 'resp_msg' field has been set, false otherwise.
      */
    public boolean hasRespMsg() {
      return fieldSetFlags()[6];
    }


    /**
      * Clears the value of the 'resp_msg' field.
      * 响应消息
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder clearRespMsg() {
      resp_msg = null;
      fieldSetFlags()[6] = false;
      return this;
    }

    /**
      * Gets the value of the 'sign' field.
      * 签名
      * @return The value.
      */
    public java.lang.String getSign() {
      return sign;
    }

    /**
      * Sets the value of the 'sign' field.
      * 签名
      * @param value The value of 'sign'.
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder setSign(java.lang.String value) {
      validate(fields()[7], value);
      this.sign = value;
      fieldSetFlags()[7] = true;
      return this;
    }

    /**
      * Checks whether the 'sign' field has been set.
      * 签名
      * @return True if the 'sign' field has been set, false otherwise.
      */
    public boolean hasSign() {
      return fieldSetFlags()[7];
    }


    /**
      * Clears the value of the 'sign' field.
      * 签名
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder clearSign() {
      sign = null;
      fieldSetFlags()[7] = false;
      return this;
    }

    /**
      * Gets the value of the 'trans_type' field.
      * 交易类型
      * @return The value.
      */
    public java.lang.String getTransType() {
      return trans_type;
    }

    /**
      * Sets the value of the 'trans_type' field.
      * 交易类型
      * @param value The value of 'trans_type'.
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder setTransType(java.lang.String value) {
      validate(fields()[8], value);
      this.trans_type = value;
      fieldSetFlags()[8] = true;
      return this;
    }

    /**
      * Checks whether the 'trans_type' field has been set.
      * 交易类型
      * @return True if the 'trans_type' field has been set, false otherwise.
      */
    public boolean hasTransType() {
      return fieldSetFlags()[8];
    }


    /**
      * Clears the value of the 'trans_type' field.
      * 交易类型
      * @return This builder.
      */
    public org.xiajinsuo.epay.sdk.RRParams.Builder clearTransType() {
      trans_type = null;
      fieldSetFlags()[8] = false;
      return this;
    }

    @Override
    public RRParams build() {
      try {
        RRParams record = new RRParams();
        record.app_id = fieldSetFlags()[0] ? this.app_id : (java.lang.String) defaultValue(fields()[0]);
        record.server_trans_id = fieldSetFlags()[1] ? this.server_trans_id : (java.lang.String) defaultValue(fields()[1]);
        record.client_trans_id = fieldSetFlags()[2] ? this.client_trans_id : (java.lang.String) defaultValue(fields()[2]);
        record.trans_timestamp = fieldSetFlags()[3] ? this.trans_timestamp : (java.lang.Long) defaultValue(fields()[3]);
        record.data = fieldSetFlags()[4] ? this.data : (java.lang.String) defaultValue(fields()[4]);
        record.resp_code = fieldSetFlags()[5] ? this.resp_code : (java.lang.String) defaultValue(fields()[5]);
        record.resp_msg = fieldSetFlags()[6] ? this.resp_msg : (java.lang.String) defaultValue(fields()[6]);
        record.sign = fieldSetFlags()[7] ? this.sign : (java.lang.String) defaultValue(fields()[7]);
        record.trans_type = fieldSetFlags()[8] ? this.trans_type : (java.lang.String) defaultValue(fields()[8]);
        return record;
      } catch (Exception e) {
        throw new org.apache.avro.AvroRuntimeException(e);
      }
    }
  }

  private static final org.apache.avro.io.DatumWriter
    WRITER$ = new org.apache.avro.specific.SpecificDatumWriter(SCHEMA$);

  @Override public void writeExternal(java.io.ObjectOutput out)
    throws java.io.IOException {
    WRITER$.write(this, SpecificData.getEncoder(out));
  }

  private static final org.apache.avro.io.DatumReader
    READER$ = new org.apache.avro.specific.SpecificDatumReader(SCHEMA$);

  @Override public void readExternal(java.io.ObjectInput in)
    throws java.io.IOException {
    READER$.read(this, SpecificData.getDecoder(in));
  }

}
